...
impl Point {
    fn parse_coordinate_str(c: &str) -> Self {
        // "x=2, y=18"
...
impl Coverage {
    fn get_gap(&self) -> i128 {
        ...
        self.ranges
            .iter()
            .for_each(|range| ranges.push(range.clone()));
...
fn main() {
    ...
    for y in 0..=limit {
        ...
        if coverage.len() != (limit + 1) {
            // have found the coverage we are looking for
