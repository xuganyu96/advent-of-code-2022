...
fn is_inbound(cube: &Cube, bounds: (i32, i32, i32, i32, i32, i32)) -> bool {
    ...
    return cube.x >= x_min && cube.x <= x_max && cube.y >= y_min
        && cube.y <= y_max && cube.z >= z_min && cube.z <= z_max;
}
...
fn is_outside(
    ...
    if droplets.contains(start) { return false; }
    ...
    while let Some(next_cube) = backlog.pop_front() {
        ...
        for neighbor in next_cube.neighbors() {
            if !footprints.contains(&neighbor) 
...
fn external_surface_area(inputs: &str, external_only: bool) {
    ...
    let surface_area = droplets.iter()
        .map(|cube| {
            return cube.neighbors().iter().map(|neighbor| {
                if !external_only && !droplets.contains(neighbor) { return 1; }
                if external_only 
