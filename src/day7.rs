...
impl Item {
    fn _to_string(&self) -> String {
        match self {
            ...
            Self::Dir(path, name, items) => {
                format!("<Dir {} {} nitems={} size={}>", path, name, items.len(), self.size())
            }
        }
    }
    ...
    fn push_file(&mut self, filename: &str, size: u64) {
        if let Self::Dir(_, _, items) = self {
            items.push(
                Rc::new(RefCell::new(Item::File(filename.to_string(), size)))
            );
        }
    }
    ...
    fn push_dir(&mut self, dirname: &str) {
        if let Self::Dir(path, _, items) = self {
            ...
            items.push(
                Rc::new(RefCell::new(Item::Dir(path.to_string(), dirname.to_string(), vec![])))
            );
        }
    }
...
pub fn solve(input: &str) {
    ...
    let root = Rc::new(RefCell::new(Item::Dir("/".to_string(), "".to_string(), vec![])));
    ...
    input.split("$ ").for_each(|cmd| {
        if cmd.len() < 2 {
            ...
            dirs.iter().for_each(|dirname| cur.borrow_mut().push_dir(dirname));
            files.iter().for_each(|(filename, size)| cur.borrow_mut().push_file(filename, *size));
